...
- [ ] 生产：周日夜定时任务自动解锁/结算（cron，未接；当前靠手动/解锁按周计算）
...
- [ ] 从零新建学科+5章+100词的 UI（当前内容靠种子/JSON 录入；本项为可选增强）
...
剩余的均为**可选增强**（非 PRD 必需）：生产级周日夜定时结算 cron、从零新建学科的录入 UI、重度游戏化动效。
