...
- **个性化（PRD §14）**：员工先填 onboarding 资料（6 项）；系统每词终评后增量更新其「标签+画像」记忆；追问/评分把 `LearnerContext`（资料+记忆）拼进提示自然个性化，不做刻意引导。打分契约：`submitNote`/`finalize` 收 `learner?`，外加 `updateMemory`。Mock 必须确定性。用 `getLearnerContext(userId)` 装配。每章学完另有「章节总结+反思追问（结合岗位）」环节（待 S3 后做）。
